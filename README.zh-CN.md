...
- **错误帧检测** — 自动标记黑帧与材质丢失帧，支持错误帧独立筛选视图
...
- Blender（任意安装方式均可：独立版、Steam、系统安装）
...
**错误帧如何检测？**
两种机制：(1) 像素分析检测黑帧（平均亮度 < 2% 且画面均匀无亮部）；(2) Blender 日志解析捕获材质丢失和 GPU 错误（"missing"、"not found"、"CUDA error" 等）。标记帧在预览卡片右上角显示红点，可通过 Errors 标签页集中查看。
